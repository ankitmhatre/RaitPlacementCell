 ...
 * authenticating accounts in the com.dragonide.raitplacementcell.samplesync domain. The
...
class Authenticator extends AbstractAccountAuthenticator {
    ...
    public Bundle getAuthToken(AccountAuthenticatorResponse response, Account account,
        ...
        final String batch= am.getUserData(account,"userdata");


        if (password != null) {

            final String authToken = NetworkUtilities.authenticate(account.name, password, batch);
            if (!TextUtils.isEmpty(authToken)) {
                final Bundle result = new Bundle();
                result.putString(AccountManager.KEY_ACCOUNT_NAME, account.name);
                result.putString(AccountManager.KEY_PASSWORD, password);
                result.putString(AccountManager.KEY_USERDATA, batch);
